<?php
$queryconfig = mysqli_query($conn, "SELECT * FROM tb_konfigurasi ORDER BY id_konfigurasi ASC LIMIT 1");
$color = mysqli_fetch_array($queryconfig);
?>
<style type="text/css">
@charset "UTF-8";
/**
...
.imagecheck-figure:before {
  ...
  -ms-user-select: none;
  user-select: none;
  background: <?php echo $color['color'];?> url("data:image/svg+xml;charset=utf8,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 8 8'%3E%3Cpath fill='%23fff' d='M6.564.75l-3.59 3.612-1.538-1.55L0 4.26 2.974 7.25 8 2.193z'/%3E%3C/svg%3E") no-repeat center center/50% 50%;
  color: #fff;
  z-index: 1;
  border-radius: 3px;
  opacity: 0;
...
.card {
  ...
  .card.card-primary {
    border-top: 2px solid <?php echo $color['color'];?>; }
  .card.card-secondary {
  ...
  .card.card-hero .card-header {
    padding: 40px;
    background-image: linear-gradient(to bottom, <?php echo $color['color'];?>, #95a0f4);
    color: #fff;
    overflow: hidden;
    height: auto;
    min-height: auto;
...
a.bb {
  text-decoration: none;
  border-bottom: 1px solid <?php echo $color['color'];?>;
  padding-bottom: 1px; }
